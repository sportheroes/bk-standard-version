...
describe('cli', function () {
  ...
  describe('CHANGELOG.md does not exist', function () {
    it('populates changelog with commits since last tag by default', function () {
      commit('✅ [ADD] first commit')
      shell.exec('git tag -a v1.0.0 -m "my awesome first release"')
      commit('✴️ [FIX] patch release')
  ...
  it('handles commit messages longer than 80 characters', function () {
    commit('✅ [ADD] (example) first commit')
    shell.exec('git tag -a v1.0.0 -m "my awesome first release"')
    commit('✴️ [FIX] (scope) this is my fairly long commit message which is testing whether or not we allow for long commit messages')
  ...
  it('formats the commit and tag messages appropriately', function () {
    ...
    shell.exec('git log --oneline -n1').stdout.should.include('[PUB] (release) 1.1.0')
    // check annotated tag message
    shell.exec('git tag -l -n1 v1.1.0').stdout.should.include('[PUB] (release) 1.1.0')
  })
  ...
  it('includes merge commits', function () {
    ...
    checkout('master')
    merge('✅ [ADD] new feature from branch', branchName)
...
describe('standard-version', function () {
  ...
  describe('dry-run', function () {
    it('skips all non-idempotent steps', function (done) {
      commit('✅ [ADD] first commit')
      shell.exec('git tag -a v1.0.0 -m "my awesome first release"')
      commit('✅ [ADD] (api) new feature!')
      execCli('--dry-run').stdout.should.match(/### ✅ Features/)
      shell.exec('git log --oneline -n1').stdout.should.include('(api) new feature!')
  ...
  describe('skip', () => {
    ...
    it('allows the commit phase to be skipped', function () {
      ...
      commit('✅ [ADD] new feature from branch')
      return execCliAsync('--skip.commit true')
        .then(function () {
          ...
          content.should.match(/new feature from branch/)
          // check last commit message
          shell.exec('git log --oneline -n1').stdout.should.include('✅ [ADD] new feature from branch')
        })
    })
  })
})
